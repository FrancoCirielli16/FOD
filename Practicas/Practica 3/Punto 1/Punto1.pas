...
procedure baja_empleado(var E:archivo;nro_empleado:integer);
var
	emp,empleado_a_buscar:empleado;
	ok:boolean;
begin
	ok:=false;
	seek(E,FilePos(E)-1);
	read(E,emp);
	seek(E,0);
	while(not eof(E))and(not(ok))do
	begin
		read(E,empleado_a_buscar);
		if(empleado_a_buscar.nro_empleado=nro_empleado)then
		begin	
			ok:=true;
			seek(E,FilePos(E)-1);
			write(E,emp);
	...
	if(ok=false)then
		...
		writeln ('SE ENCONTRO NUMERO DE EMPLEADO');
	close(E);
end;
...
begin
	...
	writeln('H-dar de baja un empleado');
	writeln('I-Finalizar');
	readln(opcion);
	while opcion <> 'I' do
	begin
		case opcion of
			...
			'I':begin
					write('Elija el numero del empleado para darlo de baja:');
					readln(nro_empleado);
					baja_empleado(E,nro_empleado);
				end;
		end;
		writeln('|--------------------------------------MENU-------------------------------------------|');
		writeln('A-Listar en pantalla los datos de empleados que tengan un nombre o apellido determinado');
